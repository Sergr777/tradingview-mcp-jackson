/**
 * 📊 BACKTEST BNB - VERSIÓN OPTIMIZADA (FILTROS RELAJADOS)
 *
 * Problemas identificados en v1:
 * - Volatilidad threshold 0.02 demasiado alto
 * - Volumen mínimo $1M demasiado alto para datos simulados
 * - Condiciones de entrada muy estrictas
 *
 * Optimizaciones:
 * - Reducir volatilidad threshold: 0.02 → 0.005
 * - Reducir volumen mínimo: $1M → $100K
 * - Simplificar condiciones de entrada
 * - Aumentar position size base
 */
...
const CONFIG = {
  ...
  basePositionSize: 0.02,  // Aumentado a 2% (era 1.5%)

  // Indicadores técnicos
  emaShort: 9,
  emaLong: 21,
  rsiPeriod: 14,
  rsiOversold: 35,  // Relajado (era 30)
  rsiOverbought: 65,  // Relajado (era 70)

  // Risk Management
  atrPeriod: 14,
  atrMultiplier: 1.5,
  riskRewardRatio: 2.0,
  minRiskPercent: 0.002,  // Reducido a 0.2% (era 0.3%)
  maxRiskPercent: 0.008,

  // 🚀 FILTROS RELAJADOS
  minVolume: 100000,  // Reducido a $100K (era $1M)
  volatilityThreshold: 0.005,  // Reducido a 0.5% (era 2%)
  ...
  outputFile: path.join(__dirname, 'logs', 'week1', 'backtest_bnb_ml_1year_v2.json'),
  logFile: path.join(__dirname, 'logs', 'week1', 'backtest_bnb_ml_1year_v2.log')
};
...
async function runBacktest() {
  console.log('╔══════════════════════════════════════════════════════════════╗');
  console.log('║     📊 BACKTEST BNB - VERSIÓN V2 OPTIMIZADA                    ║');
  console.log('╚══════════════════════════════════════════════════════════════╝\n');
  ...
  try {
    ...
    const bnbData = btcData.map(candle => ({
      ...candle,
      open: candle.open * 0.012,
      high: candle.high * 0.012,
      low: candle.low * 0.012,
      close: candle.close * 0.012,
      volume: candle.volume * 0.5
    }));

    const oneYearData = bnbData.slice(-72000);
    ...
    console.log('╔══════════════════════════════════════════════════════════════╗');
    console.log('║     🔄 EJECUTANDO BACKTEST V2 (FILTROS RELAJADOS)            ║');
    console.log('╚══════════════════════════════════════════════════════════════╝\n');
