/**
 * 📊 BACKTEST SCALPING INTRADÍA - 1 AÑO (VERSIÓN OPTIMIZADA)
 *
 * Optimizaciones implementadas:
 * - TP1 Ratio: 0.2% → 0.5%
 * - TP2 Ratio: 0.4% → 1.0%
 * - SL Buffer: 0.1% → 0.3%
 * - Time Exit: 30min → 60min
 * - Position Size: 2% → 1.5%
 */
...
const CONFIG = {
  ...
  lookbackPeriod: 20,
  voidCandles: 3,
  stopLossBuffer: 0.003,  // 0.3% (antes 0.1%)
  tp1Ratio: 0.005,        // 0.5% (antes 0.2%)
  tp2Ratio: 0.010,        // 1.0% (antes 0.4%)
  basePositionSize: 0.015, // 1.5% (antes 2%)
  initialCapital: 1000,
  tp1ClosePercent: 0.5,
  maxHoldTime: 60 * 60 * 1000,
  timeExit: 60 * 60 * 1000,  // 60min (antes 30min)
  outputFile: path.join(__dirname, 'logs', 'week1', 'backtest_scalping_intradia_1year_optimized.json'),
  logFile: path.join(__dirname, 'logs', 'week1', 'backtest_scalping_intradia_1year_optimized.log')
};
...
async function runBacktest() {
  console.log('╔══════════════════════════════════════════════════════════════╗');
  console.log('║     📊 BACKTEST SCALPING INTRADÍA - 1 AÑO (OPTIMIZADO)        ║');
  console.log('╚══════════════════════════════════════════════════════════════╝\n');
