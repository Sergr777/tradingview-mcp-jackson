/**
 * Sistema 1: Turtle Soup CTR (CORREGIDO)
 * Estrategia: Falsas rupturas de High/Low 20 períodos
 * Versión: Usa high20_corrected y low20_corrected (ventana de 20 velas ANTERIORES)
 */

export class TurtleSoupCTRCorrected {
  constructor(config = {}) {
    // PARÁMETROS BALANCEADOS (después de descubrir 10.5% de breakouts)
    this.highLowPeriod = config.highLowPeriod || 20;
    this.rsiLongThreshold = config.rsiLongThreshold || 40; // Menos estricto
    this.rsiShortThreshold = config.rsiShortThreshold || 60; // Menos estricto
    this.highLowThreshold = config.highLowThreshold || 0.001; // 0.1% (permisivo)
  ...
  detect(data, i) {
    ...
    // USAR high20_corrected y low20_corrected
    ...
    if (currentHigh > high20) {
      ...
      if (highBreakout > this.highLowThreshold &&
        ...
        return {
          ...
          system: 'TURTLE_SOUP_CTR_CORRECTED'
        };
      }
    }
