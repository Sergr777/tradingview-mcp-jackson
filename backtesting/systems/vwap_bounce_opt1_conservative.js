/**
 * VWAP Bounce - OPTIMIZACIÓN 1: CONSERVADORA
 * Cambios:
 * - Añadir filtro RSI (evitar sobrecompra/sobreventa)
 * - Añadir filtro ADX (evitar rangos laterales)
 * - Aumentar volumen confirm: 1.2x → 1.5x (más estricto)
 * - Reducir stop loss: 0.3% → 0.25% (menos riesgo)
 * - Aumentar take profit: 0.6% → 0.8% (mejor R:R)
 */

export class VWAPBounceOpt1Conservative {
  constructor(config = {}) {
    this.vwapThreshold = config.vwapThreshold || 0.001; // 0.1%
    this.volumeMultiplier = config.volumeMultiplier || 1.5; // 1.2x → 1.5x (más estricto)
    this.stopLoss = config.stopLoss || 0.0025; // 0.3% → 0.25%
    this.takeProfit = config.takeProfit || 0.008; // 0.6% → 0.8%

    // Nuevos filtros
    this.rsiLongThreshold = config.rsiLongThreshold || 60; // No LONG si RSI > 60
    this.rsiShortThreshold = config.rsiShortThreshold || 40; // No SHORT si RSI < 40
    this.adxThreshold = config.adxThreshold || 20; // Solo si ADX > 20
  ...
  detect(data, i) {
    ...
    if (vwap === null || rsi === null || adx === null) return null;

    // Filtro ADX: Evitar rangos laterales
    ...
    if (deviation > -this.vwapThreshold &&
        deviation < 0 &&
        volumeConfirm &&
        rsi < this.rsiLongThreshold) { // Filtro RSI
      ...
      return {
        ...
        target: takeProfit,
        confidence: 0.70,
        reason: `VWAP Bounce LONG - RSI: ${rsi.toFixed(1)}, ADX: ${adx.toFixed(1)}`,
        system: 'VWAP_BOUNCE_OPT1_CONSERVATIVE'
      };
    }
    ...
    if (deviation < this.vwapThreshold &&
        deviation > 0 &&
        volumeConfirm &&
        rsi > this.rsiShortThreshold) { // Filtro RSI
      ...
      return {
        ...
        target: takeProfit,
        confidence: 0.70,
        reason: `VWAP Bounce SHORT - RSI: ${rsi.toFixed(1)}, ADX: ${adx.toFixed(1)}`,
        system: 'VWAP_BOUNCE_OPT1_CONSERVATIVE'
      };
    }
