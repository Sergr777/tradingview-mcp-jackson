/**
 * Backtest individual: Turtle Soup CTR ULTRA
 * Para debugging de por qué no genera trades
 */

import { readFileSync, writeFileSync } from 'fs';
import { TurtleSoupCTRULTRA } from './systems/turtle_soup_ctr_ultra.js';

class BacktestTurtleSoupULTRA {
  constructor(config = {}) {
  ...
  run(dataFile) {
    console.log('╔════════════════════════════════════════════════════════════╗');
    console.log('║         TURTLE SOUP CTR - ULTRA PERMISIVO                   ║');
    console.log('╚════════════════════════════════════════════════════════════╝\n');
    ...
    // Ejecutar backtest
    const system = new TurtleSoupCTRULTRA();
    system.trades = [];
    system.positions = [];
    ...
    console.log('🔍 Analizando velas para detectar patrones...\n');

    for (let i = 0; i < data.timestamps.length; i++) {
      const signal = system.detect(data, i);

      // Contar estadísticas de detección
      if (i >= 20 + 14) {
        ...
        if (currentHigh > high20) highBreakouts++;
        if (currentLow < low20) lowBreakouts++;

        if (signal) {
          signalsDetected++;
          if (system.execute(signal, data, i)) {
            console.log(`   📊 Signal ${signalsDetected}: ${signal.reason}`);
          }
        }
      }
    ...
    console.log(`   - High breakouts (barridas de máximo): ${highBreakouts.toLocaleString()}`);
    console.log(`   - Low breakouts (barridas de mínimo): ${lowBreakouts.toLocaleString()}`);
    console.log(`   - Señales generadas: ${signalsDetected.toLocaleString()}`);
    console.log(`   - Trades ejecutados: ${system.trades.length}\n`);
    ...
    writeFileSync(
      'backtesting/results/turtle_soup_ultra_results.json',
      JSON.stringify(results, null, 2)
    );

    console.log('\n💾 Resultados guardados en: backtesting/results/turtle_soup_ultra_results.json');
...
// Ejecutar
const engine = new BacktestTurtleSoupULTRA();
